...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = error?.response?.data?.non_field_errors?.[0] || 'Login failed. Please try again.';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (signupData.pin !== signupData.confirmPin) {
      toast.error('PIN numbers do not match!');
      return;
    }
    ...
    if (signupData.userType === 'MERCHANT') {
      if (!signupData.registrationNumber || signupData.registrationNumber.length < 6) {
        toast.error('Valid organization registration number is required!');
        return;
      }
    ...
    if (signupData.userType === 'AGENT') {
      if (!signupData.nidNumber || signupData.nidNumber.length < 10) {
        toast.error('Valid NID number is required for agent verification!');
        return;
      }
      if (!signupData.agentCode) {
    ...
    try {
      await authAPI.register({
        ...
        password: signupData.pin,
        password2: signupData.confirmPin,
        phone_number: signupData.phone,
        email: signupData.email || undefined,
        user_type: signupData.userType,
        nid_number: signupData.nidNumber || undefined,
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <h1 className="text-2xl font-bold">Tap Pay</h1>
            </div>
            <div className="text-sm">
              {isLogin ? "New user?" : "Already have an account?"}
              <button
                onClick={() => setIsLogin(!isLogin)}
                className="ml-2 text-yellow-300 hover:text-yellow-200 font-semibold"
              >
        ...
        <div className="max-w-md mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 mt-2">
              {isLogin 
                ? 'Login to access your mobile financial services' 
            ...
            <form onSubmit={handleSignup} className="bg-white rounded-2xl shadow-xl p-8">
              <div className="mb-6">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { type: 'USER', label: 'User', icon: UserIcon, color: 'blue' },
                    { type: 'MERCHANT', label: 'Merchant', icon: BuildingStorefrontIcon, color: 'purple' },
                    { type: 'AGENT', label: 'Agent', icon: BriefcaseIcon, color: 'green' },
                  ].map((item) => (
                    <button
                      key={item.type}
                      type="button"
                      onClick={() => setSignupData({...signupData, userType: item.type})}
                      className={`py-3 px-4 rounded-lg transition-all ${
                        signupData.userType === item.type
                          ? `bg-${item.color}-600 text-white shadow-lg`
                          : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                      }`}
                    >
                      <item.icon className={`h-5 w-5 mx-auto mb-1 ${
                        signupData.userType === item.type ? 'text-white' : 'text-gray-600'
                      }`} />
